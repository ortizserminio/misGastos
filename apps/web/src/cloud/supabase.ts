...
export async function fetchShortcutEvents(db:SupabaseClient):Promise<Transaction[]>{
 const rows=check(await db.from('shortcut_events').select('event_id,amount_cents,merchant,bank,occurred_at').order('occurred_at',{ascending:false}));
 return rows.map(r=>({id:r.event_id,type:'expense',amountCents:r.amount_cents,merchant:r.merchant,bank:r.bank||'Efectivo',category:'Otros',date:String(r.occurred_at).slice(0,10),source:'shortcut',externalId:r.event_id}));
}
